...
class _CommentScreenState extends ConsumerState<CommentScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<DeleteCommentTaskState>(deleteCommentTaskControllerProvider, (previous, next) {
      if (!mounted) return;

      if (next.error != null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('ลบความคิดเห็นไม่สำเร็จ: ${next.error}')));
      ...
      if (next.error != null || next.message != null) {
        Future.delayed(const Duration(milliseconds: 300), () {
          if (mounted) {
            ref.read(deleteCommentTaskControllerProvider.notifier).reset();
          }
        });
      }
    });
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (confirmed == true) {
                                        ...
                                        if (activityId == null || activityId.isEmpty) {
                                          if (context.mounted) {
                                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ไม่พบรหัสความคิดเห็นสำหรับลบ')));
                                          }
                                          return;
                                        }
                                        await ref.read(deleteCommentTaskControllerProvider.notifier).deleteComment(activityId);
                                      }
                                    },
                                  ),
                                ),
